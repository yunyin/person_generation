...
def rename_model(config):
  ...
  with tf.Session() as sess:
    ...
    tf.logging.info(ckpt)
    saver.restore(sess, ckpt)
    tf.train.write_graph(sess.graph_def, config['logdir'], "nn_model.pbtxt", as_text=True)
#    eval_model.saver.save(sess, os.path.join(config['logdir'], "nn_model.ckpt"))
